...
#define TRRO_CONFIG_DEVICE_ID_INVALID    0x0100000A // 16777226:设备ID格式非法
#define TRRO_CONFIG_DEVICE_NAME_INVALID  0x0100000B // 16777227:设备名称格式非法
#define TRRO_CONFIG_SDK_MODE_INVALID     0x0100000C // 16777228:SDK模式值非法
#define TRRO_CONFIG_CLOUD_MODE_INVALID   0x0100000D // 16777229:云模式值非法
#define TRRO_CONFIG_REQUIRED_FIELD_MISSING 0x0100000E // 16777230:必填字段缺失
#define TRRO_CONFIG_FIELD_TYPE_ERROR     0x0100000F // 16777231:字段类型错误
#define TRRO_CONFIG_CERTIFICATE_MD5_INVALID 0x01000010 // 16777232:证书MD5校验失败
...
/**
 * @def TRRO_INIT_LICENSE_CHECK_STREAM_FAILED
 * @brief 十进制【33554452】
 * LICENSE 授权流小于设备流
 */
...
enum TrroRemoteDeviceState {
    kRemoteDeviceDisconnect = 0,
    kRemoteDeviceConnected = 1,
};
...
enum TrroColor{
    ...
    /*other color*/
     Trro_ColorBGRA = 20, /**< BGRA */
     Trro_ColorRGB = 21, /**< RGB888 */
     Trro_ColorBGR = 22, /**< BGR888 */
};
 ...
 *          gwid          视频帧的来源现场设备id
 *          stream_id     视频帧的来源现场设备流id
 *          conn_fd       接收视频流的句柄标识
 *          data          编码的视频图像帧数据
 *          len           编码的视频图像帧的长度
 *          trro_codec    编码的codec类型
 *          is_key_frame  是否是关键帧
 *          videotime     从0开始的时间戳
 * @return : void
 */
typedef void TRRO_onRemoteEncodedFrameData(void* context, const char* gwid, int stream_id, int conn_fd,
    const char* data, int len, TrroCodec trro_codec, bool is_key_frame, long long videotime);

/** 
* @name  TRRO_onRemoteDeviceState
* @brief 远端用户连接状态回调接口
* @param[in] context        回调上下文指针, 返回注册回调函数时传入的context
* @param[in] remote_device  remote端用户id
* @param[in] stream_id      remote端用户 连接的哪个steam
* @param[in] state          1、连接 0、断开 对应 TrroRemoteDeviceState, 状态可能回调多次
* @return : void
*/
...
/**
* @name TRRO_registerRemoteEncodedFrameCallback
* @brief 注册远端编码数据回调函数
* @param[in]    context              上下文
* @param[in]    callback             回调函数
* @return 1 for success, other failed
*/
extern "C" TRRO_EXPORT int TRRO_registerRemoteEncodedFrameCallback(void* context, TRRO_onRemoteEncodedFrameData* callback);
...
/**
* @name  TRRO_registerRemoteDeviceState
* @brief 注册远端用户连接状态回调接口
* @param[in]   context              上下文
* @param[in]   callback             回调函数
* @return 1 for success, other failed
*/
extern "C" TRRO_EXPORT int TRRO_registerRemoteDeviceState(void *context, TRRO_onRemoteDeviceState *callback);
...
/**
* @name  TRRO_muteLocalAudio
* @brief Mute 本地音频流
* @param[in] mute       是否mute
* @return 1 for success, other failed
*/
extern "C" TRRO_EXPORT int TRRO_muteLocalAudio(bool mute);
